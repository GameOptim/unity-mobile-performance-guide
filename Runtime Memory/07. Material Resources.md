The memory footprint of material resources is relatively small. We generally focus more on **optimizing their count**, because an excessive number of materials will increase the time cost of the `Resource.UnloadUnusedAssets` API, which will be discussed later.

An excessive number of material resources is mainly caused by redundant **Instance (clone)** materials.

This usually occurs when `MeshRender.material` parameters are accessed and modified via code. The Unity engine will instantiate a new material copy to apply changes, resulting in memory redundancy.

To solve this, optimization using **MaterialPropertyBlock** is recommended.

For specific operations and examples, refer to:

*Using MaterialPropertyBlock to Replace Material Property Operations* (full text attached below).

However, this method **is not applicable in URP**, as it will break the **SRP Batcher**.

In addition, we need to detect and optimize **suspected redundancy of non-instance materials**.

Besides quantity issues, materials often involve problems related to **texture sampling and Shader usage**, causing extra memory and GPU performance waste:

- For materials using **solid-color texture sampling**, replace the texture sampler with a color parameter to save one texture sampling cost.
- For materials with **empty texture sampling**, Unity uses a built-in texture, but the resulting color is constant and still represents waste.

![33](https://uwa-ducument-img.oss-cn-beijing.aliyuncs.com/GameOptim/2.7Material/33.png)All the above issues can be detected automatically via the **Local Resource Detection Tool**, which is much more efficient than manual inspection of massive materials.



### Appendix: *Using MaterialPropertyBlock to Replace Material Property Operations*
